...
function GraphViewInner() {
  ...
  // Auto-expand all containers on layer entry so child nodes are visible
  // immediately. Fires when Stage 1 layout is ready and no containers are
  // currently expanded (drillIntoLayer resets expandedContainers to empty).
  // A ref tracks whether we've already auto-expanded for the current
  // navigation — reset when activeLayerId changes.
  // Skip auto-expand when the layer has too many total child nodes (>500),
  // as rendering thousands of nodes at once kills performance.
  const AUTO_EXPAND_NODE_LIMIT = 500;
  const autoExpandDoneRef = useRef<string | null>(null);
  const expandedContainersForAutoExpand = useDashboardStore((s) => s.expandedContainers);
  useEffect(() => {
    if (navigationLevel !== "layer-detail") {
      autoExpandDoneRef.current = null;
      return;
    }
    if (!containerIds || containerIds.length === 0) return;
    if (layoutStatus !== "ready") return;
    if (autoExpandDoneRef.current === activeLayerId) return;
    if (expandedContainersForAutoExpand.size > 0) return;
    autoExpandDoneRef.current = activeLayerId;
    // Count total child nodes; skip if too many
    const totalChildren = detailGraph.nodes.filter((n) => n.type === "container")
      .reduce((sum, n) => sum + ((n.data as { childCount?: number }).childCount ?? 0), 0);
    if (totalChildren > AUTO_EXPAND_NODE_LIMIT) return;
    for (const cid of containerIds) {
      expandContainer(cid);
    }
  }, [navigationLevel, activeLayerId, containerIds, layoutStatus, expandContainer, expandedContainersForAutoExpand, detailGraph.nodes]);
