...
这里保存 Vgame 的只读事实索引：客户端代码结构、281 张注册配置表 Schema，以及本仓库的策划与治理文档。图谱用于定位证据和影响面，不替代真实 Excel、代码与项目 skill。
...
首次使用运行仓库内的 `驾驭工程/配置本机路径.bat`，设置本机的 `VGAME_ROOT`。脚本会自动生成 Git 忽略的 `local.env.bat`，其他路径从当前 Git 仓库和 SVN 工程推导。
