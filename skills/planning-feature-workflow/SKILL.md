...
- **Claude 负责**：需求澄清、策划案与规划、核心/专项设计、数值与配置、一致性评审、Excel 策划案、追踪矩阵与验收用例；必要时承担代码/配置落地（`IMPLEMENTATION`）。
- **Codex 负责**：UI 与 Figma 资产的最终产出（HTML 原型、页面/组件 PNG、可编辑 Figma 源文件、Handoff Manifest）。
- 涉及 UI/Figma 的档位（`UI_ONLY` / `DOC_UI` / `FIGMA_UI` / `DOC_FIGMA`）：Claude 产出 UI 规格交接（`03-specialist-results/ui-spec.md` 或 `figma-ui-plan.md`）交给 Codex；除非用户明确要求，Claude 不自行手搓 UI/Figma 资产。
- **UI 示意图先评审再入案**：Codex 先产出 **UI 示意图** 交用户评审，用户确认方向后（门禁 `ui_mockup_approved`）才把页面主图并入 Excel 策划案，再进入正式交付与验收。
...
- 适用档位：`UI_ONLY`、`DOC_UI`。
- 责任：**由 Codex 执行**（按 Claude 的 `ui-spec.md` 交接），加载 `vgame-ui-prototype`；已安装时同时加载 `$frontend-design` 作为视觉工艺基线。Claude 仅在用户明确要求时亲自制作。
- **先出 UI 示意图交用户评审**，用户确认方向后置 `ui_mockup_approved=true`，才进入正式资产与 Excel 嵌图。
...
- 责任：**由 Codex 执行**（按 Claude 的 `figma-ui-plan.md` 交接），加载 `vgame-figma-ui`；该 skill 会调用官方 Figma 写入、页面搭建和组件库能力，并应用游戏 UI 约束。Claude 仅在用户明确要求时亲自制作。
- **先出 UI 示意图交用户评审**（`ui_mockup_approved`），方向确认后再进入正式 Figma 写入与 Excel 嵌图。
...
- 责任：`doc-writer`。
- UI/Figma 档位：嵌入页面主图前必须已取得 `ui_mockup_approved`（UI 示意图经用户评审）。
...
- UI 示意图未经用户评审（`ui_mockup_approved`）就把页面图并入 Excel 策划案。
